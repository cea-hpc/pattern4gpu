...
computeMatAndSync1(Func func, CellMaterialVariableScalarRef<DataType> var, eVarSyncVersion vs_version) {
  computeMatAndSyncOnEvents1<Func, DataType>(UniqueArray<Ref<ax::RunQueueEvent>>() /*liste vide d'événements*/,
      func, var, vs_version);
}
...
template<typename Func, typename DataType>
void VarSyncMng::
computeMatAndSyncOnEvents1(ArrayView<Ref<ax::RunQueueEvent>> depends_on_evts,
      ...
      vs_version == VS_bulksync_evqueue) 
  {
    ...
    if (vs_version == VS_bulksync_std) {
      ...
      this->multiMatSynchronize1(ref_queue, var);
    }
    PROF_ACC_END;
      ...
      vs_version == VS_overlap_evqueue_d) 
  {
    ...
    if (vs_version == VS_overlap_evqueue) {
      ...
      this->multiMatSynchronize1(m_ref_queue_bnd, var);
