...
void VarSyncMng::multiMatSynchronize1(Ref<RunQueue> ref_queue, 
...
#else
  // m_buf_addr_mng gère des zones mémoires HOST/DEVICE pré-allouées
  MultiEnvVarHD<DataType> menv_var(var_menv, m_buf_addr_mng);
  m_buf_addr_mng->asyncCpyHToD(*(ref_queue.get()));
  auto d_menv_var = menv_var.spanD();
#endif
    ...
    MeshVariableSynchronizerList& vars)
{
  ...
  if (m_nb_nei==0 || nb_var==0) {
    return;
  }

  // Asynchronous pointers tranfer onto device
  vars.asyncHToD(*(ref_queue.get()));
  ...
  for(Integer inei=0 ; inei<m_nb_nei ; ++inei) {
    ...
    // On amorce la réception sur l'HOTE
    auto byte_buf_rcv = buf_rcv_h.multiView(inei).rangeView(); // le buffer de réception pour inei
    requests[inei] = m_pm->recv(byte_buf_rcv, rank_nei, /*blocking=*/false);
    msg_types[inei] = inei+1; // >0 pour la réception
  }
  ...
  for(Integer inei=0 ; inei<m_nb_nei ; ++inei) {
    ...
    auto byte_buf_snd_d = buf_snd_d.multiView(inei); // le buffer d'envoi pour inei sur le DEVICE
    auto byte_buf_snd_h = buf_snd_h.multiView(inei); // le buffer d'envoi pour inei sur l'HOTE
    ...
    for(Integer ivar=0 ; ivar<nb_var ; ++ivar) {
      ...
      lvars[ivar]->asyncPackIntoBuf(owned_evi_pn[inei], byte_buf_var_d, *(ref_queue.get()));
    }
  ...
  for(Integer inei=0 ; inei<m_nb_nei ; ++inei) {
    ...
    // On amorce l'envoi
    auto byte_buf_snd = buf_snd_h.multiView(inei).rangeView(); // le buffer d'envoi pour inei
    requests[m_nb_nei+inei] = m_pm->send(byte_buf_snd, rank_nei, /*blocking=*/false);
    msg_types[m_nb_nei+inei] = -inei-1; // <0 pour l'envoi
  }
  ...
  while(nb_pending_rcv>0) {
    ...
    for(Integer idone_req : done_indexes) {
      if (pending_types[idone_req] > 0) { // >0 signifie que c'est une requête de reception
        ...
        // on transfère les donnees de façon asynchrone pour le inei-ième voisin
        {
          auto byte_buf_rcv_h = buf_rcv_h.multiView(inei); // buffer des données reçues sur l'HOTE
          auto byte_buf_rcv_d = buf_rcv_d.multiView(inei); // buffer des données reçues à transférer sur le DEVICE
          ...
          for(Integer ivar=0 ; ivar<nb_var ; ++ivar) {
            ...
            lvars[ivar]->asyncUnpackFromBuf(ghost_evi_pn[inei], byte_buf_var_d, *(ref_queue.get()));
          }
        }
      }
      is_done_requests[idone_req] = true;
...
#define INST_VAR_SYNC_MNG_MULTI_MAT_SYNCHRONIZE(__DataType__) \
  template void VarSyncMng::multiMatSynchronize1(Ref<RunQueue> ref_queue, CellMaterialVariableScalarRef<__DataType__> var)
